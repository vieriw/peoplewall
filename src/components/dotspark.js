...
class DotSpark{
    constructor(scene,VShader, FShader, z, position, scale, index,slower) {
      this.scene = scene;
      this.slower= slower;
    ...
    init(discard) {
     ...
     // const indices = new Uint16Array(numVisible);
      ...
      const timeDiffs= new Float32Array(numVisible);
    ...
    geometry.setAttribute('timeDiff', new THREE.InstancedBufferAttribute(timeDiffs, 1, false));
    
     
    this.object = new THREE.Mesh( geometry , this.ShaderMaterial  );
    //  this.object2 = new THREE.Mesh( bgPlaneGeometry , this.ShaderMaterial  );  
  // this.object.position.set( 1400,-440,0)
  
    this.object.position.copy(this.position)
     
    //4.5 
  // this.object.scale.set(8.5 ,8.5 ,8.5 );
   this.object.scale.set(this.scale,this.scale,this.scale);
    ...
    entryAnimate(target,target2){
      ...
      this.object.position.set(-2400-Math.random() * 200, -Math.random() * 0,0.3)  
         ...
         .delay(10)
         .to( {x: target.x},8000+this.slower)
      ...
      // .easing(TWEEN.Easing.Linear.None)
         ...
         .onComplete( ()=> {
             ...
             tl2.delay(600+this.slower*2).start()
      })
      const tl2 = new TWEEN.Tween(this.object.position)
      .to({x: target2.x},6000+this.slower)
      .easing(TWEEN.Easing.Linear.None)
       .onComplete( ()=> {
